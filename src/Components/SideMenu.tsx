...
const SideMenu: React.FC<SideMenuProps> = ({ userIsLog }) => {
    ...
    return (
        <div className={`fixed flex-row w-fit lg:w-18 2xl:w-22 h-fit left-0 top-1/2 transform -translate-y-1/2 gap-x-1 z-50 transition ease-in-out delay-150 duration-300 ${userIsLog ? 'flex' : 'hidden'} ${menuVisible ? 'translate-x-0' : '-translate-x-12 lg:-translate-x-16 2xl:-translate-x-20'}`}>
            ...
            <div className='bg-custom-gold w-5 2xl:w-8 flex items-center justify-center rounded-r-lg  ' onClick={toggleMenu}>
                <img src="arrow.svg" alt="" className='w-5 h-5 2xl:w-8 2xl:h-8' />
            </div>
        </div>
    );
};
